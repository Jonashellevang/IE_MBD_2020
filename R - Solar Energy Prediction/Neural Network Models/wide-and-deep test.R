...
X_test <- as.array(as.matrix(data[Date > 20051231, c(100:137)]))
y_test <- as.array(as.matrix(data[Date > 20051231, c(2:99)]))
...
features <- rbind(X_train, X_validation)
target <- as.numeric(c(y_train[,1], y_validation[,1]))

# train model
model %>% fit(
  x = list(rbind(X_train, X_validation)),
  y = list(c(y_train[,1], y_validation[,1])),
  epochs = 100,
  batch_size = 64,
